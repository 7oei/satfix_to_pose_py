...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        if flag == 0 and lat !=0.00000 and lon != 0.00000:
            base_lat = lat
            base_lon = lon
            rospy.loginfo("基準lat: %.7f, lon: %.7f", lat, lon)

            flag = 1
        if flag >= 1:
            rospy.loginfo("lat: %.7f, lon: %.7f", lat, lon)
            ...
            br.sendTransform((r_x, r_y, 0),tf.transformations.quaternion_from_euler(0, 0, 0),rospy.Time.now(),"gnss_link","map")
